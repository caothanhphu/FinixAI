...
def run_update_exchange_rates():
    ...
    # 0. Kiểm tra kết nối DB ban đầu và dừng nếu thất bại
    try:
        # Gọi create_tables_postgres để đảm bảo nó cũng kiểm tra kết nối
        # và có thể raise lỗi nếu không kết nối được
        database.create_tables_postgres()
    except Exception as e: # Bắt lỗi từ get_db_connection nếu nó được raise
        # Lỗi đã được in trong get_db_connection hoặc create_tables_postgres
        print("Thoát chương trình do không thể khởi tạo cơ sở dữ liệu.")
    ...
    fetched_rates, source_update_time_str = scraper.fetch_exchange_rates_from_vcb_api()
